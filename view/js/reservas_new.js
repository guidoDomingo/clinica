                ...
                if (response.status === 'success' && response.data && response.data.length > 0) {
                    ...
                    response.data.forEach(function(item) {
                        ...
                        $('#tablaReservasPorFecha tbody').append(
                            '<tr data-reserva-id="' + item.reserva_id + '">' +
                            ...
                            '<td class="text-center"><span class="' + estadoClass + ' small">' + item.reserva_estado + '</span></td>' +
                            '</tr>'
                        );
...
/**
 * Verificar si DataTables Buttons está disponible y todas las extensiones requeridas
 * @returns {boolean} - True si DataTables Buttons está disponible, False en caso contrario
 */
function verificarDisponibilidadDatatablesBotones() {
    ...
    // Verificar si Buttons está disponible
    if (typeof $.fn.dataTable.Buttons === 'undefined') {
        console.error('DataTables Buttons no está disponible');
        return false;
    }
    
    // Verificar si la extensión print está disponible
    if (typeof $.fn.dataTable.ext.buttons.print === 'undefined') {
        console.error('DataTables Buttons Print no está disponible');
        return false;
    }
    
    console.log('Todas las extensiones de DataTables necesarias están disponibles');
    return true;
}

/**
 * Inicializa botones de acción básicos para la tabla de reservas sin usar la extensión Buttons
 */
function inicializarBotonesReservas() {
    // Si ya existe el contenedor de botones, lo eliminamos
    $('.botones-tabla-reservas').remove();
    
    // Crear un contenedor de botones básico
    const botonesHTML = `
        <div class="botones-tabla-reservas mb-2">
            <div class="btn-group btn-group-sm">
                <button type="button" class="btn btn-outline-info btn-sm btn-refresh-tabla">
                    <i class="fas fa-sync-alt"></i> Actualizar
                </button>
                <button type="button" class="btn btn-outline-primary btn-sm btn-imprimir-tabla">
    ...
    // Insertar los botones antes de la tabla
    $(botonesHTML).insertBefore('#tablaReservasPorFecha');
    
    // Evento para actualizar la tabla
    $('.btn-refresh-tabla').on('click', function() {
        const fecha = $('#fechaReservaNew').val();
        cargarReservasPorFecha(fecha);
    });
    
    // Evento para imprimir la tabla (versión básica)
    $('.btn-imprimir-tabla').on('click', function() {
        const tabla = $('#tablaReservasPorFecha').clone();
        const fecha = $('#fechaReservaNew').val() || 'Actual';
        const fechaFormateada = moment(fecha).format('DD/MM/YYYY');
        ...
        const contenidoImpresion = `
            <html>
                <head>
                    <title>Reservas del ${fechaFormateada}</title>
                    <style>
                ...
                <body>
                    <h1>Reservas para el día ${fechaFormateada}</h1>
                    <div>${tabla.prop('outerHTML')}</div>
                </body>
            </html>
        `;
...
/**
 * Debug function to check all DataTables dependencies
 * Can be called from the browser console for troubleshooting
 */
function debugDataTablesExtensions() {
    console.group('DataTables Dependencies Check');
    
    // Check core DataTables
    console.log('DataTables core:', typeof $.fn.dataTable !== 'undefined' ? '✅ Available' : '❌ Missing');
    
    // Check Buttons base
    console.log('DataTables Buttons base:', typeof $.fn.dataTable.Buttons !== 'undefined' ? '✅ Available' : '❌ Missing');
    
    // Check individual button types
    const buttonTypes = ['print', 'csv', 'excel', 'pdf', 'copy', 'colvis'];
    buttonTypes.forEach(type => {
        console.log(`Button type '${type}':`, 
            typeof $.fn.dataTable.ext.buttons[type] !== 'undefined' ? '✅ Available' : '❌ Missing');
    });
    
    // Check dependencies
    console.log('JSZip (for Excel export):', typeof window.JSZip !== 'undefined' ? '✅ Available' : '❌ Missing');
    console.log('pdfMake (for PDF export):', typeof window.pdfMake !== 'undefined' ? '✅ Available' : '❌ Missing');
    
    console.groupEnd();
    
    // Return recommendation
    if (typeof $.fn.dataTable === 'undefined') {
        return 'DataTables core is missing. Check if jquery.dataTables.min.js is loaded.';
    } else if (typeof $.fn.dataTable.Buttons === 'undefined') {
        return 'DataTables Buttons extension is missing. Check if dataTables.buttons.min.js is loaded.';
    } else if (typeof $.fn.dataTable.ext.buttons.print === 'undefined') {
        return 'Print button extension is missing. Check if buttons.print.min.js is loaded.';
    } else if (typeof $.fn.dataTable.ext.buttons.excel === 'undefined' || typeof window.JSZip === 'undefined') {
        return 'Excel export is not available. Check if buttons.html5.min.js and jszip.min.js are loaded.';
    }
    
    return 'All DataTables dependencies appear to be available!';
}