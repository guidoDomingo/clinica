...
function guardarPersona() {
    ...
    const personaData = {
        ...
        department_id: formData.get('perDpto') !== '0' ? parseInt(formData.get('perDpto')) : null,
        city_id: formData.get('perCity') !== '0' ? parseInt(formData.get('perCity')) : null,
        is_minor: formData.get('perMenor') === 'true',
        guardian_name: formData.get('perMenor') === 'true' ? formData.get('perTutor') : null,
        guardian_document: formData.get('perMenor') === 'true' ? formData.get('perDocTutor') : null,
        is_active: true
    };
...
function actualizarPersona() {
    ...
    const personaData = {
        ...
        department_id: formData.get('EditperDpto') !== '0' ? parseInt(formData.get('EditperDpto')) : null,
        city_id: formData.get('EditperCity') !== '0' ? parseInt(formData.get('EditperCity')) : null,
        is_minor: formData.get('EditperMenor') === 'true',
        guardian_name: formData.get('EditperMenor') === 'true' ? formData.get('EditperTutor') : null,
        guardian_document: formData.get('EditperMenor') === 'true' ? formData.get('EditperDocTutor') : null
    };
    ...
    .then(data => {
        if (data.person_id) {
            ...
            mostrarAlerta('error', data.message || 'Error al actualizar la persona');
        }
    })
    .catch(error => {
        console.error('Error:', error);
        ...
        if (result.isConfirmed) {
            ...
            .then(data => {
                if (data.message) {
                    ...
                    mostrarAlerta('error', data.message || 'Error al eliminar la persona');
                }
            })
            .catch(error => {
                console.error('Error:', error);
...
function cambiarEstadoPersona() {
    ...
    .then(data => {
        if (data.person_id) {
            mostrarAlerta('success', `Persona ${activar ? 'activada' : 'desactivada'} correctamente`);
            tablaPersonas.ajax.reload();
        } else {
            mostrarAlerta('error', data.message || `Error al ${activar ? 'activar' : 'desactivar'} la persona`);
        }
    })
    .catch(error => {
        console.error('Error:', error);
        ...
        let html = `
            <div class="row">
                <div class="col-md-4 text-center">
                    <img src="${data.profile_photo ? 'view/uploads/profile/' + data.profile_photo : 'view/dist/img/user-default.jpg'}" 
...
function subirFotoPerfil(personId, file) {
    ...
    .then(data => {
        if (data.message) {
            mostrarAlerta('success', 'Persona guardada y foto subida correctamente');
            $('#modalAgregarPersonas').modal('hide');
            $('#modalEditarPersonas').modal('hide');
            tablaPersonas.ajax.reload();
        } else {
            mostrarAlerta('warning', data.message || 'La persona se guardó pero hubo un error al subir la foto');
            $('#modalAgregarPersonas').modal('hide');
            $('#modalEditarPersonas').modal('hide');
            tablaPersonas.ajax.reload();
        }
    ...
    .catch(error => {
        console.error('Error:', error);
        mostrarAlerta('warning', 'La persona se guardó pero hubo un error al subir la foto');
        $('#modalAgregarPersonas').modal('hide');
        $('#modalEditarPersonas').modal('hide');
        tablaPersonas.ajax.reload();
    });
...
function toggleCamposTutor() {
    ...
    document.getElementById('divTutor').style.display = esmenor ? 'block' : 'none';
    document.getElementById('divDocTutor').style.display = esmenor ? 'block' : 'none';
}
...
function toggleCamposTutorEdit() {
    ...
    document.getElementById('divEditTutor').style.display = esmenor ? 'block' : 'none';
    document.getElementById('divEditDocTutor').style.display = esmenor ? 'block' : 'none';
}
...
function validarFormularioPersona() {
    ...
    if (esmenor) {
        ...
        if (tutor === 'N/A' || !docTutor) {
            mostrarAlerta('warning', 'Para menores de edad, debe completar la información del tutor');
            return false;
        }
    }
